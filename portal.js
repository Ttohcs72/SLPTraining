...
function renderQuizInto(node, quizData, passCallback, classId){
  ...
  container.innerHTML=`
    ...
    <div class="lesson-header"><span class="pill">Pass: 9/10 (you may miss 1)</span><span class="pill">Name & Store required</span></div>
  ...
  quizData.forEach((q,idx)=>{ const [question,options]=q; const block=document.createElement('div'); block.className='quiz-question'; const shuffled=shuffleArray(options);
    ...
    shuffled.forEach((opt,oi)=>{ const label=document.createElement('label'); label.innerHTML=`<input type="radio" name="q${idx}" value="${opt.replace(/"/g,'&quot;')}" /> ${opt}`; opts.appendChild(label); });
  ...
  qForm.addEventListener('submit',(e)=>{
    ...
    let correct=0; quizData.forEach((q,idx)=>{ const radios=qForm.querySelectorAll(`input[name="q${idx}"]`); let sel=null; radios.forEach(r=>{if(r.checked) sel=r.value;});
      const answer=q[1][0]; if(sel===answer) correct+=1; });
    ...
    if(passed){ if(classId){ setCompleted(classId); setLastSlide(classId,TOTAL_SLIDES-1);} const cert=document.createElement('div'); cert.className='certificate'; const today=new Date().toLocaleDateString();
      cert.innerHTML=`<h1>Certificate of Completion</h1><div class="subtitle">Awarded for successful completion with a score of 90% or higher.</div>
      ...
      result.appendChild(cert); if(typeof passCallback==='function'){ passCallback({name,store,score}); } }
    else{ const review=document.createElement('div'); review.className='print-note'; review.textContent='Review the slides above and retake the quiz when ready.'; result.appendChild(review); }
  });
  node.appendChild(container); node.appendChild(result);
}
function renderClass(slides, quizData, classId){
  const root=document.getElementById('lesson_root'); const topBar=document.createElement('div'); topBar.className='top-progress'; topBar.innerHTML=`<div id="top_prog" class="top-progress-inner"></div>`; root.appendChild(topBar);
  const nav=document.createElement('div'); nav.className='navbar'; nav.innerHTML=`<div class="counter"><span id="slide_counter">Slide 1 of 13</span></div><div class="arrows"><button id="prev_btn" class="iconbtn" disabled>← Prev</button><button id="next_btn" class="iconbtn">Next →</button></div>`; root.appendChild(nav);
  const deck=document.createElement('div'); deck.id='slide_host'; root.appendChild(deck);
  let idx=Math.max(0,getLastSlide(classId)+1); if(isNaN(idx)||idx<0||idx>12) idx=0;
  function update(){
    const prog=document.getElementById('top_prog'); const pct=Math.round(((Math.min(idx,12))/12)*95); if(prog) prog.style.width=(idx>=12?'95%':pct+'%');
    document.getElementById('slide_counter').textContent=(idx<12)?`Slide ${idx+1} of 13`:'Slide 13 of 13 — Quiz';
    document.getElementById('prev_btn').disabled=(idx===0); document.getElementById('next_btn').disabled=(idx>=12);
    deck.innerHTML=''; if(idx<12){ const s=slides[idx]; const node=document.createElement('div'); node.className='slide'; node.innerHTML=`<h2>${idx+1}. ${s.h}</h2>${s.body}`; deck.appendChild(node); setLastSlide(classId,idx); }
    else{ renderQuizInto(deck, quizData, function(){}, classId); const p=document.getElementById('top_prog'); if(p) p.style.width='100%'; }
  }
  document.getElementById('prev_btn').addEventListener('click',()=>{ if(idx>0){ idx--; update(); } });
  document.getElementById('next_btn').addEventListener('click',()=>{ if(idx<12){ idx++; update(); } });
  update();
}
